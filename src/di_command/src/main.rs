...
fn main() {
    ...
    let input_key = &args[2];
    let before_left_clip = &args[3];

    let cursor_pos = before_left_clip.len() as usize;
    ...
    let mut right_clip = clipboard.chars().skip(cursor_pos).take(clipboard.len()).collect::<String>();

    let right_clip_len = right_clip.len() as usize;
...
fn trim_strings(clipboard: String, end: usize, target: &String) -> String {
    ...
    for i in 0..end {
        if clipboard.chars().skip(i).take(1).collect::<String>() == *target {
            res = clipboard.chars().skip(i).take(clipboard.len()).collect::<String>();
            break
        }
    }
